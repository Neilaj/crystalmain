...
interface ImagePickerProps {
  open: boolean;
  onClose: () => void;
  onSelect: (url: string, alt?: string, title?: string, caption?: string) => void;
}
...
export function ImagePicker({ open, onClose, onSelect }: ImagePickerProps) {
  ...
  // Step 2: Image details before inserting
  ...
  // Select image → go to details step
  const selectForDetails = useCallback((url: string, filename?: string) => {
    setSelectedImage({
      url,
      alt: "",
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          fetchMedia();
          selectForDetails(data.url, file.name);
        } else {
      ...
      <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
          <h2 className="text-lg font-semibold text-gray-900">
            {selectedImage ? "Image Details" : "Insert Image"}
          </h2>
          <button
            onClick={() => {
              if (selectedImage) {
              ...
              <div className="flex-1 space-y-3">
                <div>
                  ...
                  <p className="mt-1 text-[10px] text-gray-400">
                    Required for accessibility & SEO. Describes the image content to search engines and screen readers.
                  </p>
                </div>
                ...
                <div>
                  <label className="mb-1 flex items-center gap-1.5 text-xs font-medium text-gray-700">
                    Title
                    <span className="rounded bg-blue-100 px-1 py-0.5 text-[9px] font-bold text-blue-600">
                      Optional
                    </span>
                  </label>
                  <input
                ...
                <>
                  {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-16 text-center">
                      ...
                      <p className="mt-3 text-sm text-gray-500">No images uploaded yet</p>
                      <button
                    ...
                    <div className="grid grid-cols-4 gap-3 max-h-[300px] overflow-y-auto">
                      {media.map((item) => (
                        <button
                          key={item.id}
                          onClick={() => selectForDetails(item.url, item.filename)}
                    ...
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter" && urlInput.trim()) {
                          selectForDetails(urlInput.trim());
                        }
                      }}
                    />
                    <button
                      onClick={() => {
                        if (urlInput.trim()) selectForDetails(urlInput.trim());
                      }}
